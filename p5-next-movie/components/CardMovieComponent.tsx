...
export default function CardMovieComponent({ params }: CardMovieProps) {
  const {
    poster_path,
    backdrop_path,
    title,
  ...
  if (poster_path !== null) image_path = getMoviePicture(poster_path);
  return (
    <Link href={`/movie/${id}`} className="card-link">
      <Card className="card-item bg-stone-900 text-destructive-foreground hover:bg-stone-800 cursor-pointer select-none">
        ...
        ></Image>
        <CardHeader>
          <CardTitle title={title} className="text-xl title-content">
            {title}
          </CardTitle>
        ...
        <CardContent className="h-28   ">
          <div className=" max-h-full card-content">{overview}</div>
        </CardContent>
